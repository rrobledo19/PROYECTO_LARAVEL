...
class controller_credito_maestro extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            return response()->json(['status' => true, 'id_inserted' => $credito_maestro->id_credito, 'Genial!'], 200);
    ...
    public function storeDetalleCredito(Request $request, $id_credito){
        try{
            
            $datos_detalle = json_decode($request->getContent());

            for ($datos_detalle as $detalle){
            ...
            Log::critical('No se pudo almacenar Credito  '.$e->getCode().', '.$e->getLine().', '.$e->getMessage());
            return response(' [x_x]: Oh Oh! Algo ha salido mal.', 500);
        }
    }
