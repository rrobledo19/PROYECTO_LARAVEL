...
class controller_pagosmaestro extends Controller
{
    ...
    public function mincouta($id_credito){
                  $query = DB::table('credito_detalle')
                     ->select('id_credito','fecha_credito','nro_coutas','estado',DB::raw('count(*) as contar'))
                     ->where ('id_credito',$id_credito)
                     ->where('nro_coutas','>',0)
                     ->where('estado','=','P')
                     ->take(1)
                     ->groupBy('id_credito','fecha_credito','nro_coutas','estado')
                     ->get();

                     return response()->json_decode($query);
